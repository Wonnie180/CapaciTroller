class Drop {
  ...
  Drop(int x_neg, int x_pos) {
    this.x  = random(x_neg, x_pos); // random x position ie width because anywhere along the width of screen
  ...
  void fall(float velocidad) { // function  to determine the speed and shape of the drop 
    ...
    yspeed = (yspeed + grav) * velocidad; // speed increases as gravity acts on the drop
